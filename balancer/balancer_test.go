...
var _ = Describe("Balancer", func() {
	...
	It("should initialize with defaults", func() {
		...
		Expect(subject.selector[0].Addr()).To(Equal("127.0.0.1:7481"))
	})
	...
	Describe("Next", func() {
		...
		BeforeEach(func() {
			...
			subject.selector = pool{
				&redisBackend{opt: mockOpts("127.0.0.1:7481"), up: 0, connections: 0, latency: ms},
				&redisBackend{opt: mockOpts("127.0.0.1:7482"), up: 1, connections: 10, latency: 2 * ms},
				&redisBackend{opt: mockOpts("127.0.0.1:7483"), up: 1, connections: 8, latency: 3 * ms},
				&redisBackend{opt: mockOpts("127.0.0.1:7484"), up: 1, connections: 14, latency: ms},
			}
		})

		It("should pick next backend (first-up)", func() {
			subject.mode = ModeFirstUp
			Expect(subject.pickNext().Addr).To(Equal("127.0.0.1:7482"))
			Expect(subject.pickNext().Addr).To(Equal("127.0.0.1:7482"))
			Expect(subject.pickNext().Addr).To(Equal("127.0.0.1:7482"))
			Expect(subject.pickNext().Addr).To(Equal("127.0.0.1:7482"))
		...
		It("should pick next backend (least-conn)", func() {
			...
			Expect(subject.pickNext().Addr).To(Equal("127.0.0.1:7483"))
			Expect(subject.pickNext().Addr).To(Equal("127.0.0.1:7483"))
			Expect(subject.pickNext().Addr).To(Equal("127.0.0.1:7482"))
			Expect(subject.pickNext().Addr).To(Equal("127.0.0.1:7483"))
			Expect(subject.pickNext().Addr).To(Equal("127.0.0.1:7482"))
		...
		It("should pick next backend (min-latency)", func() {
			...
			Expect(subject.pickNext().Addr).To(Equal("127.0.0.1:7484"))
			Expect(subject.pickNext().Addr).To(Equal("127.0.0.1:7484"))
			Expect(subject.pickNext().Addr).To(Equal("127.0.0.1:7484"))
			Expect(subject.pickNext().Addr).To(Equal("127.0.0.1:7484"))
		...
		It("should fallback on random when everything down", func() {
			...
			Expect(subject.pickNext().Addr).To(Equal("127.0.0.1:7484"))
			Expect(subject.pickNext().Addr).To(Equal("127.0.0.1:7481"))
			Expect(subject.pickNext().Addr).To(Equal("127.0.0.1:7481"))
			Expect(subject.pickNext().Addr).To(Equal("127.0.0.1:7481"))
			Expect(subject.pickNext().Addr).To(Equal("127.0.0.1:7483"))
		})
